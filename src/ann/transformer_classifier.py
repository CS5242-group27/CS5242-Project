...
def main():
    model = make_cls(src_dim=1000, seq_len=30, cls_num=10, N=2)
    print(model)

    # Input: (batch, seq_len, token_dim)
    # After embedding: (batch, seq_len, d_model)
    # Transformer block output: (batch, seq_len, d_model) -> (batch, seq_len * d_model)
    # Classifier output: (batch, class_num)
    a = torch.zeros(size=(2, 30, 1000), dtype=torch.float)
    b = model(a, src_mask=None)
    print(b.shape)
